...
class Pix2PixModel(torch.nn.Module):
    ...
    def forward(self, src_data, ref_data, mode):
        ...
        if mode == 'dyeing':
            with torch.no_grad():
                fake_image = self.netG.dyeing(src_semantics,
                    src_image, ref_semantics , ref_image, src_data['path'])
            return fake_image

        elif mode == 'styling':
            with torch.no_grad():
                fake_image = self.netG.styling(src_semantics,
                    src_image, ref_semantics, ref_data['path'])
            return fake_image
